...
No uploads. No accounts. No tracking. OpenDocs Studio ships **14 production-ready tools** for working with images and PDFs — every operation happens locally on your device using the Canvas API, pdf-lib and pdf.js. Your files never leave your machine.

| | |
|---|---|
| 🧰 **14 tools** | 8 image · 6 PDF |
...
| **Image Compressor** | Shrink file size by lowering quality and/or reformatting | Format (auto / JPG / PNG / WebP), quality 1–100 |
| **Image Format Converter** | Clean re-encode between formats | JPG, PNG or WebP output with quality control |
...
| PDF manipulation | pdf-lib (merge, split, rotate, compress, images → PDF) |
| PDF rendering | pdfjs-dist (PDF → canvas → PNG/JPG) |
...
│   ├── registry.ts             #   All 14 tool definitions (inputs + form fields)
...
│   └── pdfProcessors.ts        #   merge · split · rotate · images→PDF · compress · render
├── components/
...
**14 tools · 0 uploads · 100% client-side** ✅
